    ...
    def get_states(self, K, P_perp, Lambda_perp):
        ...
        for sector in self.sectors:
            ...
            for pair in itertools.combinations(
                quarks.index.to_numpy(), r=sector.count("q")
            ):
                fermi_indices.append(pair)

            # antiquarks
            for pair in itertools.combinations(
                antiquarks.index.to_numpy(), r=sector.count("qbar")
            ):
                antifermi_indices.append(pair)

            # gluons
            for pair in itertools.combinations_with_replacement(
                gluons.index.to_numpy(), r=sector.count("g")
            ):
                bose_indices.append(pair)

            # Check momentum sums
            for pair in itertools.product(
                fermi_indices, antifermi_indices, bose_indices
            ):
                nested_lists = [list(inner_tuple) for inner_tuple in pair]
                indices = [item for sublist in nested_lists for item in sublist]
                if (
                    qnums.loc[indices, "n"].sum() == K
                    and qnums.loc[indices, "n⟂"].sum() == P_perp
                ):
                    valid_states.append(
                        op.Fock(
                            nested_lists[0],
                            nested_lists[1],
                            self.postprocess_bosons(nested_lists[2]),
                        )
                    )

        return op.FockSum(valid_states)
